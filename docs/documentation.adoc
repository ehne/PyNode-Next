...
Returns the node's position that was set. Note that it will not return the position of the node if it has not been set.

[IMPORTANT]
====
`node.position()` has not been implemented in PyNode Next as yet.
